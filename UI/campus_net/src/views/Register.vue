        ...
        <div id="register_form">
            ...
            <div id="form_body_wrapper">
                <div class="field">
                    ...
                    <input v-model="user.scid" @blur="scidValidator($event)" name="scid" type="text" placeholder="SCID" >
                </div>
                <div class="field">
                    <label for="name">用户名</label>
                    <input v-model="user.username" @blur="usernameValidator($event)" name="username" type="text" placeholder="Username" >
                    <!-- 昵称字母开头，不能以数字开头，可以中文 -->
                </div>
                <div class="field">
                    <label for="password">密码</label>
                    <input v-model="user.pwd" @blur="pwdValidator($event)" name="password" type="password" placeholder="Password" >
                </div>
                <div class="field">
                    <label for="password_again">再次输入密码</label>
                    <input v-model="pwdAgain" @blur="pwdValidator" name="password_again" type="password" placeholder="Password Again" >
                </div>
                <ul id="error_list">
                    <li v-for="(error, name, index) in formError" :key="index" v-if="error.flag">
                        {{error.description.zh_CN}} <br/>
...
export default class Register extends Vue {
    ...
    private formError = {
        scidInvalid: { flag: false, description: { en_US: 'SCID Invalid', zh_CN: '学号不存在或已绑定账号' } },
        passwordMismatch: { flag: false, description: { en_US: 'Passwords Mismatch', zh_CN: '两次输入密码不一致' } },
        usernameInvalid: { flag: false, description: { en_US: 'Username Invalid', zh_CN: '用户名非法' } },
    };
    private user = {
        scid: null,
        username: null,
